...
type Props={scopeKey:string;entries:LogEntry[];total:number;locale:'zh-CN'|'en-US';page:number;pageSize:number;onPageChange:(page:number)=>void;onPageSizeChange:(size:number)=>void;onFilter:(field:string,value:unknown,exclude:boolean)=>void};
...
export function parseEmbeddedJSON(value:unknown,depth=0):unknown{
  ...
  if(typeof value==='string'){
    ...
    try{return parseEmbeddedJSON(JSON.parse(trimmed),depth+1)}catch{return value}
  }
  if(Array.isArray(value))return value.map(item=>parseEmbeddedJSON(item,depth+1));
  if(value!==null&&typeof value==='object')return Object.fromEntries(Object.entries(value as Record<string,unknown>).map(([key,item])=>[key,parseEmbeddedJSON(item,depth+1)]));
  return value;
}

/** Recursively renders an expandable JSON tree with filterable leaf values. */
function JsonNode({name,value,nodeId,fieldPath=name,selectedId,onSelect,expandDepth,depth=0}:{name:string;value:unknown;nodeId:string;fieldPath?:string;selectedId?:string;onSelect:(event:MouseEvent<HTMLButtonElement>,id:string,field:string,value:unknown)=>void;expandDepth:number;depth?:number}){
  ...
  if(complex){const entries=Object.entries(value as Record<string,unknown>);const array=Array.isArray(value);return <div className="json-node" style={{paddingLeft:depth*12}}><button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open}>{open?'▾':'▸'}</button><span className="json-key">{name}:</span><span className="json-brace">{array?'[':'{'}</span>{open&&<div>{entries.map(([key,item])=>{const childPath=array?`${fieldPath}[${key}]`:fieldPath?`${fieldPath}.${key}`:key;return <JsonNode key={key} name={key} value={item} nodeId={`${nodeId}.${key}`} fieldPath={childPath} selectedId={selectedId} onSelect={onSelect} expandDepth={expandDepth} depth={depth+1}/>})}</div>}<span className="json-brace">{array?']':'}'}</span></div>}
  return <div className="json-leaf" style={{paddingLeft:depth*12+18}}><span className="json-key">{name}:</span><button type="button" className={`${typeof value==='number'?'json-number':'json-value'} json-filter-value${selectedId===nodeId?' selected':''}`} onClick={event=>onSelect(event,nodeId,fieldPath,value)}>{typeof value==='string'?`"${value}"`:String(value)}</button></div>
}

/** Renders field controls, pagination, raw JSON records, and the table result view. */
export default function LogResults({scopeKey,entries,total,locale,page,pageSize,onPageChange,onPageSizeChange,onFilter}:Props){
  ...
  const [expandDepthByScope,setExpandDepthByScope]=useState<Record<string,number>>({});
  ...
  useEffect(()=>{if(!jsonSettingsOpen)return;const close=(event:PointerEvent)=>{if(!jsonSettingsRef.current?.contains(event.target as Node))setJsonSettingsOpen(false)};document.addEventListener('pointerdown',close,true);return()=>document.removeEventListener('pointerdown',close,true)},[jsonSettingsOpen]);
  ...
  const setExpandDepth=(value:number)=>setExpandDepthByScope(current=>({...current,[scopeKey]:Math.max(0,Math.min(8,value))}));
  const labels={search:zh?'搜索字段名称':'Search fields',display:zh?'显示字段':'Display fields',all:zh?'全部字段':'All fields',noFields:zh?'没有匹配的字段':'No matching fields',copy:zh?'复制当前日志 JSON':'Copy log JSON',copyValue:zh?'复制':'Copy',copied:zh?'已复制':'Copied',include:zh?'加入筛选':'Add to filter',exclude:zh?'排除':'Exclude',collapse:zh?'收起字段栏':'Collapse fields',expand:zh?'展开字段栏':'Expand fields',table:zh?'表格':'Table',original:zh?'原始':'Raw',time:zh?'时间':'Time',jsonSettings:zh?'JSON 展开设置':'JSON expansion settings',expandLevel:zh?'默认展开层级':'Default expansion depth',currentStore:zh?'仅应用于当前日志库':'Current logstore only',reset:zh?'恢复默认':'Reset default',levelUnit:zh?'层':'levels',perPage:zh?'每页显示':'Per page',total:zh?'总数':'Total',jump:zh?'到第':'Go to',pageUnit:zh?'页':'page',confirm:zh?'确认':'Go',empty:zh?'暂无日志，请先运行查询':'No logs. Run a query first.'};
